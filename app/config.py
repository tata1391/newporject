...
def validate_config() -> None:
    ...
    if not settings.test_mode:
        raise RuntimeError("TEST_MODE باید برای این پروژه روی true باشد.")
    if settings.max_test_requests < 1:
        raise ValueError("MAX_TEST_REQUESTS باید حداقل 1 باشد.")
    if settings.request_delay < 0:
        raise ValueError("REQUEST_DELAY نمی‌تواند منفی باشد.")
